...
struct data : public primitive_base<data> {
    ...
    data(const primitive_id& id, memory::ptr mem, const primitive_id& ext_prim_id = "", std::shared_ptr<ov::Node> ov_node = nullptr)
        : primitive_base(id, {}, ext_prim_id, padding(), optional_data_type(), ov_node), mem(mem) {}
