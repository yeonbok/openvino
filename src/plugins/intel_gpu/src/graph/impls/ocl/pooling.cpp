...
void validate_args(const pooling_node& arg) {
    auto const& input_rank = arg.input().get_output_layout().get_spatial_rank();
    auto const& output_rank = arg.get_output_layout().get_spatial_rank();
    auto const& stride_rank = arg.get_primitive()->stride.size();
    auto const& window_rank = arg.get_primitive()->size.size();
