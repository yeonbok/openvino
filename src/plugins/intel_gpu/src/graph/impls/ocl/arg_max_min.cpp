...
public:
    static primitive_impl* create(const arg_max_min_node& arg, const kernel_impl_params& impl_param) {
        ...
        const auto& outputs_num = primitive->num_outputs;//primitive->input.size() == 3 ? 2 : 1;  // second output passed as input for TOP_K layer
        ...
        if (outputs_num == 2) {
            argm_params.outputs.push_back(convert_data_tensor(impl_param.output_layouts[0])); // The 2nd output layout is same as 1st output
        }
