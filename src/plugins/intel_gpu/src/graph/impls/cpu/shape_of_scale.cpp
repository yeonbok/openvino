...
struct shape_of_scale_impl : public typed_primitive_impl<shape_of_scale> {
    using parent = typed_primitive_impl<shape_of_scale>;
    using parent::parent;

    std::string variable_id;
    bool calculated = false;
    DECLARE_OBJECT_TYPE_SERIALIZATION(cldnn::cpu::shape_of_scale_impl)

    std::unique_ptr<primitive_impl> clone() const override {
        return make_unique<shape_of_scale_impl>(*this);
    }

    shape_of_scale_impl() : parent("shape_of_scale_cpu_impl") {}

    explicit shape_of_scale_impl(const shape_of_scale_node& outer) {
        set_node_params(outer);
    }

    void set_node_params(const program_node& arg) override {
        OPENVINO_ASSERT(arg.is_type<shape_of_scale>(), "[GPU] Incorrect program_node type");
    }
...
public:
    static std::unique_ptr<primitive_impl> create(const shape_of_scale_node& arg, const kernel_impl_params& impl_param) {
        return make_unique<shape_of_scale_impl>();
    }
};
...
attach_shape_of_scale_impl::attach_shape_of_scale_impl() {
    ...
    implementation_map<shape_of_scale>::add(impl_types::cpu, shape_types::static_shape, shape_of_scale_impl::create, types, formats);
    implementation_map<shape_of_scale>::add(impl_types::cpu, shape_types::dynamic_shape, shape_of_scale_impl::create, types, formats);
}
