...
struct broadcast_impl : public typed_primitive_impl<broadcast> {
    ...
    event::ptr add_to_cmd_list_impl(command_list* list, const std::vector<event::ptr>& event, typed_primitive_inst<broadcast>& instance) override {
        return execute_impl(event, instance);
    }

    void update_command_impl(command_list* list, const std::vector<event::ptr>& event, typed_primitive_inst<broadcast>& instance) override { }
