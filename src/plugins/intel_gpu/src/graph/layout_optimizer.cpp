...
static size_t get_post_ops_count(const program_node& node) {
    ...
    for (auto& fo : node.get_fused_primitives()) {
        if (fo.is_type<activation>() || fo.is_type<eltwise>()) {
            ...
            // pre-scale, pre-shift
            if (fo.get_type_params<kernel_selector::quantize_params>()->per_tensor_input_scale
                && fo.get_type_params<kernel_selector::quantize_params>()->per_tensor_input_shift) {
                onednn_post_ops_count++;
            } else {
            ...
            if (fo.get_type_params<kernel_selector::quantize_params>()->has_post_scale
                && fo.get_type_params<kernel_selector::quantize_params>()->has_post_shift
                && fo.get_type_params<kernel_selector::quantize_params>()->per_tensor_output_scale
                && fo.get_type_params<kernel_selector::quantize_params>()->per_tensor_output_shift) {
                onednn_post_ops_count++;
            } else {
                onednn_post_ops_count += 2;
            }
            ...
            auto out_range_usage = fo.get_type_params<kernel_selector::quantize_params>()->per_tensor_output_range
                                   && (fo.get_type_params<kernel_selector::quantize_params>()->out_lo
                                   < fo.get_type_params<kernel_selector::quantize_params>()->out_hi);
            ...
            if (out_range_usage) {
                ...
                // clamp
                if (fo.get_type_params<kernel_selector::quantize_params>()->has_clamp) {
                    onednn_post_ops_count++;
                }
            } else {
                // clamp
...
bool layout_optimizer::can_fuse_reorder(program_node& prev, program_node& next, format fmt_prev, format fmt_next) {
    ...
    if (next.is_type<convolution>() && use_onednn_impls && fmt_prev == format::bfyx &&
        next_output_layout.feature() >= 16 && prev_output_layout.feature() == 1)
        return true;
...
bool layout_optimizer::needs_onednn_bfyx_to_blocked(format fmt_prev, format fmt_next, layout& prev_output_layout, const convolution_node& node) {
    ...
    if (fmt_prev == format::bfyx &&
        next_output_layout.feature() >= 16 && prev_output_layout.feature() <= 4 && prev_output_layout.feature() >= 2 &&
        ...
        for (auto& fo : node.get_fused_primitives()) {
            if (fo.is_type<eltwise>()) {
                ...
                if ((out_layout.count() == in_layout.count()) &&
                    ...
                    fo.typed_desc<eltwise>()->needs_onednn_sum_post_op(in_layout)) {
...
layout layout_optimizer::get_expected_layout(layout const& current_layout,
    ...
    bool onednn_valid_params = onednn_valid_dt &&
                               ...
                               input_layout.batch() < 16;  // oneDNNs optimized kernel doesn't support big batches yet
    ...
    if (use_onednn_impls && onednn_valid_params && spatial_dims_num <= 3) {
        ...
        expected_tensor = current_layout.size;
        auto input_layout = node.get_dependency(0).get_output_layout();
        int input_features = input_layout.feature();
        int output_features = current_layout.feature();
    ...
    if (!_forcing_map.empty() && _forcing_map.count(node.id()) != 0) {
        ...
        if (!node.get_fused_primitives().empty()) {
            ...
            for (auto& fused_op : node.get_fused_primitives()) {
                if (fused_op.is_type<eltwise>() && node.get_dependencies().size() > fused_op.dep_start_idx && fused_op.deps.size() == 1) {
                    ...
                    if (program_helpers::are_layouts_identical_for_onednn_sum_post_op(eltw_in.get_output_layout(), node.get_output_layout()) &&
                        fused_op.typed_desc<eltwise>()->needs_onednn_sum_post_op(eltw_in.get_output_layout())) {
        ...
        if (node.is_type<deconvolution>()) {
            ...
            bool valid_ic = input_layout.feature() >= 16;
            bool valid_groups = deconv.get_primitive()->groups == 1;
            bool onednn_valid_post_ops = get_post_ops_count(node) <= 32;
            bool valid_batch = input_layout.batch() < 16;  // oneDNN's optimized kernel doesn't support big batches yet
        ...
        for (auto& fo : node.get_fused_primitives()) {
            if (fo.is_type<eltwise>()) {
                ...
                auto in_dt = in_layout.data_type;
                auto out_dt = out_layout.data_type;
                if (fo.typed_desc<eltwise>()->needs_onednn_sum_post_op(in_layout)) {
                    if ((out_layout.count() == in_layout.count()) &&
                        (data_type_traits::is_floating_point(in_dt) || data_type_traits::is_floating_point(out_dt)) && in_dt != out_dt) {
                        impl_candidate = impl_types::ocl;
                        break;
...
format layout_optimizer::get_preferred_format(program_node& node) {
    ...
    if (!_forcing_map.empty() && _forcing_map.count(node.id()) != 0) {
        ...
        if (use_onednn_impls && input.get_output_layout().feature() == 1 && input.is_in_data_flow() && !input.is_constant()) {
            auto in_layout = input.get_output_layout();
            if (in_layout.format == format::bfyx && output_layout.feature() >= 16 && expected != format::bfyx)
        ...
        if (only_gemm_users(node)) {
            ...
            if (use_onednn_impls && layout.batch() % 32 == 0) {
                if (node.get_users().size() == 1 && node.get_users().front()->is_type<convolution>()) {
                    ...
                    auto ws = conv.get_dependency(1).get_output_layout();
