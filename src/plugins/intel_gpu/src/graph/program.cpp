...
std::pair<int64_t, int64_t> program::get_estimated_device_mem_usage() {
    ...
    for (const auto& node : nodes_to_allocate) {
        ...
        bool is_igpu = (engine.get_device_info().dev_type == cldnn::device_type::integrated_gpu);
        if (is_igpu)
            total_host_alloc_size += engine.get_used_device_memory(allocation_type::usm_device);
        std::cout << "total_host_alloc_size= " << total_host_alloc_size << ", max_global_mem_size = " << max_global_mem_size << std::endl;
