        ...
        if (node->is_type<eltwise>()) {
            ...
            if (eltw->stride.empty() && !node->get_users().empty()) {
                ...
                for (auto user : users) {
                    ...
                    if (user->get_dependency_new(0).first->id() != eltw->id) {
                        can_shrink = false;
                        break;
                    }
                ...
                if (can_shrink) {
                    ...
                    for (size_t dep = 0; dep < e->input_size(); dep++) {
                        ...
                        auto dep_info = node->get_dependency_new(dep);
                        if (dep_info.first->get_output_layout(dep_info.second).size.spatial[0] == 1) {
                            dep_stride_x = 1;
                        }

                        if (dep_info.first->get_output_layout(dep_info.second).size.spatial[1] == 1) {
                            dep_stride_y = 1;
                        }
