...
void post_optimize_weights::optimize_weights(T& node, program& p) {
    ...
    for (auto i = offsets.weights_offset; i < offsets.bias_offset; i++) {
        auto weights = node.get_dependency_new(i);
        auto& weights_node = *weights.first;
        auto weights_layout = weights_node.get_output_layout(weights.second);
        ...
        for (auto& reorder : reorders) {
            ...
            auto g_node_info = node.get_dependency(i);
            auto& g_node = *g_node.first;
