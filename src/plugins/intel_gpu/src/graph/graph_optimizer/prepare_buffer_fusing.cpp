...
//                    if (!fused_op.node->as<eltwise>().get_primitive()->needs_onednn_sum_post_op(eltw_in_layout))
    ...
    auto def_fmt = layout::get_default_format(node.get_output_layout().get_rank(), false, false);
...
void concat_in_place_optimization::optimize_cascade(concatenation_node& node, std::list<concatenation_node*>& need_reoptimization) {
    ...
    if (concat_axis_legacy >= 2) {
        ...
        auto spatial_size = std::max<size_t>(node.get_output_layout().get_rank(), 4) - 2;
    ...
    if (node.can_be_optimized()) {
        lower_padd[concat_axis_legacy] = node.get_output_layout().data_padding.lower_size().sizes()[concat_axis_legacy];
        upper_padd[concat_axis_legacy] = node.get_output_layout().data_padding.upper_size().sizes()[concat_axis_legacy];
    }
    node.set_output_padding(padding(lower_padd, upper_padd));

    upper_padd[concat_axis_legacy] += node.get_output_layout().get_dims()[concat_axis];
