...
//%ODO
    for (auto a : _outputs) {
        if (eng.is_the_same_buffer(*mem_new, *a)) {
            return;
        }
    }
...
event::ptr primitive_inst::execute(const std::vector<event::ptr>& events) {
    ...
    if (_exec_deps_new.empty())
        return _impl->execute(events, *this);

    std::vector<event::ptr> dependencies;
    dependencies.reserve(_exec_deps_new.size());
    for (auto& input : _exec_deps_new) {
...
void primitive_inst::build_deps() {
    if (_deps_new.empty() && !_node.get_dependencies_new().empty()) {
        ...
        for (const auto& n : _node.get_dependencies_new()) {
            dep_nodes.push_back(n.first);
            dep_idxes.push_back(n.second);
        }
        //_deps_new = _network.get_primitives(dep_nodes);
        auto prims = _network.get_primitives(dep_nodes);
        for (int32_t i = 0; i < prims.size(); ++i) {
            _deps_new.push_back(std::make_pair(prims[i], dep_idxes[i]));
        }
        _exec_deps_new = build_exec_deps(_deps_new);
#endif
...
void primitive_inst::allocate_internal_buffers(void) {
    ...
    const auto& inst_deps = _network.get_primitives(_node.get_dependencies_new());
...
memory::ptr primitive_inst::allocate_output(engine& _engine, memory_pool& pool, const program_node& _node,
    ...
    const auto& node_deps = _node.get_dependencies_new();
...
//    if (is_internal && (_node.can_be_optimized() || _node.is_type<generic_layer>())) {
    if (is_internal && (_node.can_be_optimized())) { // TODO (temporarily omitted)
        GPU_DEBUG_IF(debug_config->verbose >= 2) {
...
//    } else if (is_internal && _node.is_output() && _node.is_type<generic_layer>() &&
    } else if (is_internal && _node.is_output() && // TODO (temporarily omitted)
...
//                _node.get_memory_dependencies(),
                ...
                alloc_type,
//                true);
                false); // TODO (taylor) turn on this once required memory reuse support is enabled for multiple output
    }
}
