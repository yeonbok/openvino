...
//    convert_fused_activation_func_params(arg, params.activations); // TODO (taylor)
    ...
    for (auto& fused_prim : arg.get_fused_primitives()) {
        ...
        for (size_t i = desc.dep_idx_start; i < desc.dep_idx_start + desc.dep_size; i++) {
            desc.tensors.push_back(convert_data_tensor(arg.get_dependency_new(i).first->get_output_layout()));
        }
