...
public:
    ...
    int32_t input_idx(size_t idx = 0) const { return get_dependency(idx).second; }
};
...
public:
    static layout calc_output_layout(custom_gpu_primitive_node const& node, int32_t idx) {
        ...
        if (output_layout.format == format::any) {
            output_layout.format = node.get_dependency(0).first->get_output_layout().format;
        }
