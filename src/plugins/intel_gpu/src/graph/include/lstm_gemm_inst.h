...
public:
    ...
    program_node& hidden() const { return bias_term() ? *get_dependency(4).first : *get_dependency(3).first; }
...
public:
    static layout calc_output_layout(lstm_gemm_node const& node, int32_t idx);
