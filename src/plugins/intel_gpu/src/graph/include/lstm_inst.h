...
public:
    ...
    program_node& recurrent() const { return *get_dependency_new(2).first; }
    program_node& bias() const { return *get_dependency_new(3).first; }
    program_node& inital_hidden() const { return *get_dependency_new(bias_term() ? 4 : 3).first; }
    program_node& inital_cell() const {
        // This doesn't scale. We should use a map to get the dependencies index at primitive level
        return *get_dependency_new(bias_term() ? (initial_hidden_term() ? 5 : 4) : (initial_hidden_term() ? 4 : 2)).first;
    }
    program_node& peepholes() const { return *get_dependency_new(6).first; }
