...
struct primitive_type_base : primitive_type {
    ...
    std::vector<cldnn::layout> calc_output_layouts(const cldnn::program_node& node, const kernel_impl_params& impl_param,
                                                   std::map<int, memory::ptr> constant_data) const override {
        if (node.type() != this)
            throw std::invalid_argument("primitive_type_base::calc_output_layouts: primitive type mismatch");

        return typed_primitive_inst<PType>::calc_output_layouts(node, impl_param, constant_data);
    }
