...
public:
    ...
    program_node& weights(size_t idx = 0) const {
        ...
        return *get_dependency_new(1 + idx).first;
    }
    ...
    program_node& bias(size_t idx = 0) const {
        ...
        return *get_dependency_new(1 + this->get_split() + idx).first;
    }
...
public:
    ...
    program_node& input() const { return *get_dependency_new(0).first; }
    program_node& trans() const { return *get_dependency_new(1).first; }
    program_node& mask() const { return *get_dependency_new(2).first; }
