...
public:
    ...
    program_node& dyn_length() const { return *get_dependency_new(1).first; }
    program_node& weights() const { return *get_dependency_new(2).first; }

    program_node& bias() const {
        CLDNN_ERROR_BOOL(id(), "Bias term", !bias_term(), "Trying to get non existing bias.");
        return *get_dependency_new(3).first;
    }
