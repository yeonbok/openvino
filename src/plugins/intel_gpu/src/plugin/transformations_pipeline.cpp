...
void TransformationsPipeline::apply(std::shared_ptr<ov::Model> func) {
    ...
    bool unroll_loop = config.get_enable_loop_unrolling();
    {
        ...
        kv_cache_config.keyCacheQuantBychannel = config.get_key_cache_quant_mode() == ov::intel_gpu::CacheQuantMode::BY_CHANNEL;
        kv_cache_config.keyCacheGroupSize = config.get_key_cache_quant_mode() == ov::intel_gpu::CacheQuantMode::BY_CHANNEL ? 16 : 0;
        ...
        manager.register_pass<ov::pass::ConvertPagedAttnInputs>(kv_cache_config,
            [&infer_precision](const ov::element::Type& precision,
               ...
               int64_t& block_size) {
                ...
                if (bychannel) {
                    std::cout << "@@@@@@@@@@@@@@ cache by channel!!! " << std::endl;
                    ...
                    std::cout << "@@@@@@@@@@@@@@ cache by hidden!!! " << std::endl;
                    if (precision == ov::element::i8 || precision == ov::element::u8) {
