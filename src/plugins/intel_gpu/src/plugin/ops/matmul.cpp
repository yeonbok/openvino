            ...
            for (size_t i = 0; i < inputPrimitives.size(); ++i) {
                ...
                if (inputDimsN != outDimsN || inputDimsN < 4) {
                    ...
                    if (inputDimsN == 1) {
                        ...
                        // * If rank of the **first** input is equal to 1, it is always unsqueezed to 2D tensor **row
                        // vector** (regardless of `transpose_a`) by adding axes with size 1 at ROW_INDEX_DIM, to the
                        // **left** of the shape. For example `[S]` will be reshaped to `[1, S]`.
                        // * If rank of the **second** input is equal to 1, it is always unsqueezed to 2D tensor
                        // **column vector** (regardless of `transpose_b`) by adding axes with size 1 at COL_INDEX_DIM,
                        // to the **right** of the shape. For example `[S]` will be reshaped to `[S, 1]`.
                        ...
                        // Specs says that shapes must be unsqueezed regardless of tranpose flag, but primitive
                        // implementation always respects transposes so we have to swap dimensions correspondingly to
