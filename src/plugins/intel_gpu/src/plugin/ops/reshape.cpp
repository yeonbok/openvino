...
static void CreateCommonReshapeOp(Program& p, const std::shared_ptr<ngraph::Node>& op) {
    ...
    auto reshapeOp = std::dynamic_pointer_cast<ngraph::op::v1::Reshape>(op);
    if (in_shape.is_dynamic()) {
        auto reshapePrim = cldnn::reshape(layerName, reshapeInputId, inputPrimitives[1], reshapeOp->get_special_zero(), out_shape, op->get_friendly_name());
