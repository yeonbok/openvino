...
UnsqueezeBroadcastReshapeSDPAFusion::UnsqueezeBroadcastReshapeSDPAFusion() {
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        std::cout << "Match " << m.get_match_root()->get_friendly_name() << "\n";
        ...
        if (target_shape_constant_b == target_shape_constant_c) {
            std::cout << "False3 for " << m.get_match_root()->get_friendly_name() << "\n";
            return false;
        }
