    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (i == 1) {
            ...
            if (ind_input.is_dynamic()) {
                ...
                size_t dim_offset = last_idx < 2 ? last_idx : last_idx + num_shape_info_dim - newParams.indices_rank;
                auto indices_last_dim = toCodeString(dims[last_idx], dim_offset + (newParams.inputs[0].is_dynamic() ? num_shape_info_dim : 0));
