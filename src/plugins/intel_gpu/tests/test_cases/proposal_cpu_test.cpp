...
extern float output_probabilities[];
...
// Second output
//-----------------------------------------------------------------------------
const primitive_id output_layer_name1 = "permute1";
const primitive_id output_layer_name2 = "permute2";

template <typename Dtype, typename ImInfoType = Dtype>
class TestRunnerProposal2ndOutput
{
    public:
        explicit TestRunnerProposal2ndOutput(cldnn::tensor image_info_size);

        std::pair<memory::ptr, memory::ptr> Run(std::vector<Dtype>& data,
                                                std::vector<Dtype>& rois);
...
template <typename Dtype, typename ImInfoType>
TestRunnerProposal2ndOutput<Dtype, ImInfoType>::TestRunnerProposal2ndOutput(cldnn::tensor image_info_size) :
                            _cls_scores_layout(cldnn::type_to_data_type<Dtype>::value, format::bfyx, { 1, 18, 23, 14 } ),
                            _bbox_pred_layout(cldnn::type_to_data_type<Dtype>::value, format::bfyx, { 1, 36, 23, 14 } ),
                            _image_info_layout(cldnn::type_to_data_type<ImInfoType>::value, format::bfyx, image_info_size),
                            _test_layer(layer_name,
                                        ...
                                        padding())
{
    ...
    _topology.add(input_layout(image_info_name, _image_info_layout));
    _topology.add(_test_layer);
    _topology.add(permute(output_layer_name1, {input_info(layer_name, 0)}, {0, 1, 2, 3}));
    _topology.add(permute(output_layer_name2, {input_info(layer_name, 1)}, {0, 1, 2, 3}));

    _network.reset(new network(get_test_engine(), _topology));
}

template <typename Dtype, typename ImInfoType>
std::pair<memory::ptr, memory::ptr> TestRunnerProposal2ndOutput<Dtype, ImInfoType>::Run(std::vector<Dtype>& cls_scores_vals,
                                                                std::vector<Dtype>& bbox_pred_vals)
{
    auto& engine = get_test_engine();
    memory::ptr cls_scores = engine.attach_memory(_cls_scores_layout, cls_scores_vals.data());
    ...
    std::map<primitive_id, network_output> network_output = _network->execute();
    EXPECT_EQ(network_output.begin()->first, output_layer_name1);

    return std::make_pair(network_output.at(output_layer_name1).get_memory(), network_output.at(output_layer_name2).get_memory());
}

TEST(proposal_multiple_output, basic) {
    std::vector<float> cls_scores(&cls_scores_data[0], &cls_scores_data[cls_scores_data_size]);
    std::vector<float> bbox_pred(&bbox_pred_data[0], &bbox_pred_data[bbox_pred_data_size]);

    TestRunnerProposal2ndOutput<float> t({ 1, 3, 1, 1 });
    std::pair<memory::ptr, memory::ptr> network_output = t.Run(cls_scores, bbox_pred);

    // First output
    auto out_mem1 = network_output.first;
    cldnn::mem_lock<float> output1(out_mem1, get_test_stream());
    ASSERT_EQ(output1.size(), proposal_ref_size);

    for (size_t i = 0; i < proposal_ref_size; i++) {
        EXPECT_NEAR(output1[i], proposal_ref[i], epsilon);
    }

    // Second output
    auto out_mem2 = network_output.second;
    cldnn::mem_lock<float> output2(out_mem2, get_test_stream());
    ASSERT_EQ(output2.size(), post_nms_topn);

    for (size_t i = 0; i < post_nms_topn; i++) {
        EXPECT_NEAR(output2[i], output_probabilities[i], epsilon);
    }
}

TEST(proposal_multiple_output, fp16) {
    std::vector<FLOAT16> cls_scores(&cls_scores_data[0], &cls_scores_data[cls_scores_data_size]);
    std::vector<FLOAT16> bbox_pred(&bbox_pred_data[0], &bbox_pred_data[bbox_pred_data_size]);

    TestRunnerProposal2ndOutput<FLOAT16> t({ 1, 3, 1, 1 });
    std::pair<memory::ptr, memory::ptr> network_output = t.Run(cls_scores, bbox_pred);

    // First output
    auto out_mem1 = network_output.first;
    cldnn::mem_lock<FLOAT16> output1(out_mem1, get_test_stream());
    ASSERT_EQ(output1.size(), proposal_ref_size);

    for (size_t i = 0; i < proposal_ref_size; i++) {
        FLOAT16 ref(proposal_ref[i]);
        EXPECT_NEAR((float)output1[i], (float)ref, epsilon_fp16);
    }

    // Second output
    auto out_mem2 = network_output.second;
    cldnn::mem_lock<FLOAT16> output2(out_mem2, get_test_stream());
    ASSERT_EQ(output2.size(), post_nms_topn);

    for (size_t i = 0; i < post_nms_topn; i++) {
        FLOAT16 ref(output_probabilities[i]);
        EXPECT_NEAR((float)output2[i], (float)ref, epsilon_fp16);
    }
}

TEST(proposal_multiple_output, scores_fp16_im_info_fp32) {
    std::vector<FLOAT16> cls_scores(&cls_scores_data[0], &cls_scores_data[cls_scores_data_size]);
    std::vector<FLOAT16> bbox_pred(&bbox_pred_data[0], &bbox_pred_data[bbox_pred_data_size]);

    TestRunnerProposal2ndOutput<FLOAT16, float> t({ 1, 3, 1, 1 });
    std::pair<memory::ptr, memory::ptr> network_output = t.Run(cls_scores, bbox_pred);
...
TEST(proposal_multiple_output, scores_fp32_im_info_fp16) {
    std::vector<float> cls_scores(&cls_scores_data[0], &cls_scores_data[cls_scores_data_size]);
    std::vector<float> bbox_pred(&bbox_pred_data[0], &bbox_pred_data[bbox_pred_data_size]);

    TestRunnerProposal2ndOutput<float, FLOAT16> t({ 1, 3, 1, 1 });
    std::pair<memory::ptr, memory::ptr> network_output = t.Run(cls_scores, bbox_pred);
    ...
    for (size_t i = 0; i < proposal_ref_size; i++) {
        float ref(proposal_ref[i]);
        EXPECT_NEAR((float)output1[i], (float)ref, epsilon);
    }
    ...
    for (size_t i = 0; i < post_nms_topn; i++) {
        float ref(output_probabilities[i]);
        EXPECT_NEAR((float)output2[i], (float)ref, epsilon);
    }
}

TEST(proposal_multiple_output, img_info_batched) {
    std::vector<float> cls_scores(&cls_scores_data[0], &cls_scores_data[cls_scores_data_size]);
    std::vector<float> bbox_pred(&bbox_pred_data[0], &bbox_pred_data[bbox_pred_data_size]);

    TestRunnerProposal2ndOutput<float> t({ 2, 3, 1, 1 });
    std::pair<memory::ptr, memory::ptr> network_output = t.Run(cls_scores, bbox_pred);
...
TEST(proposal_multiple_output, img_info_batch_only) {
    std::vector<float> cls_scores(&cls_scores_data[0], &cls_scores_data[cls_scores_data_size]);
    std::vector<float> bbox_pred(&bbox_pred_data[0], &bbox_pred_data[bbox_pred_data_size]);

    TestRunnerProposal2ndOutput<float> t({ 3, 1, 1, 1 });
    std::pair<memory::ptr, memory::ptr> network_output = t.Run(cls_scores, bbox_pred);
