...
TEST_P(eltwise_quantize, u8) {
    ...
    create_topologies(
        ...
        quantize("quantize", input_info("eltwise"), input_info("in_lo"), input_info("in_hi"), input_info("out_lo"), input_info("out_hi"), 256, data_types::u8),
...
TEST_P(eltwise_no_pitches_same_dims_quantize, quantize_f32_output) {
    ...
    create_topologies(
        ...
        data("out_lo", {get_mem(get_single_element_layout(p), -128)}),
        data("out_hi", {get_mem(get_single_element_layout(p), 127)}),
        quantize("quantize", input_info("eltwise"), input_info("in_lo"), input_info("in_hi"), input_info("out_lo"), input_info("out_hi"), 256, p.input_type),
        reorder("out", input_info("quantize"), p.default_format, data_types::f32)
    );
