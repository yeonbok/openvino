...
private:
    ...
    void cast_vector(std::vector<OUT_T>& output_vector, size_t num_elements) const {
        ...
        if (std::numeric_limits<IN_T>::has_infinity && std::numeric_limits<OUT_T>::has_infinity) {
            IN_T inf_in = std::numeric_limits<IN_T>::infinity();
            OUT_T inf_out = std::numeric_limits<OUT_T>::infinity();
            for (size_t i = 0; i < output_size; i++) {
                IN_T val_in = first[i];
                if (val_in == -inf_in)
                    output_vector.push_back(-inf_out);
                else if (val_in == inf_in)
                    output_vector.push_back(inf_out);
                else
                    output_vector.push_back(static_cast<OUT_T>(val_in));
            }
        } else {
...
#    pragma warning(disable : 4804)
                ...
                if (!std::is_same<OUT_T, IN_T>::value) {
                    if (std::numeric_limits<IN_T>::is_signed && std::numeric_limits<OUT_T>::lowest() > c) {
                        std::cout << "element::from<IN_T>() : " << element::from<IN_T>() << std::endl;
                        std::cout << "std::numeric_limits<IN_T>::is_signed : " << std::numeric_limits<IN_T>::is_signed << std::endl;
                        std::cout << "std::numeric_limits<OUT_T>::lowest() : " << std::numeric_limits<OUT_T>::lowest() << std::endl;
                        std::cout << "Cannot cast vector from " << Type << " constant to "
                                    << element::from<OUT_T>() << ". Some values are outside the range. Example: " << c << std::endl;
