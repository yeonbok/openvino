...
| `KEY_CLDNN_ENABLE_LOOP_UNROLLING` | `YES` / `NO`             | `YES`             | Turning this key on enables to unroll recurrent layers such as TensorIterator or Loop with fixed iteration count. By default, it is turned on. Note that unrolling loops will cause increase of clDNN build time as much as the iteration count, becuase the kernels consiting a body network are created and built for all iterations. On the other hand, disabling unrolling for loops with small iteration count and small body network might perform worse than unrolling, because the overhead for handling subnetworks might be more significant in overall performance. Thus, this key should be set by considering the iteration count and body network's slze.|
